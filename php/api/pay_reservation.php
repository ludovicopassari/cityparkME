...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($bearer_token) {
        try {
            ...
            if (isset($data['id_user']) && isset($data['reservation_id'])) {
                $id_user = $data['id_user'];
                $id_user = $data['reservation_id'];

                // Prepara la query per recuperare i pagamenti in base a id_user
                $sql_reservation = "INSERT INTO payments (reservation_id, payment_method, payment_date, amount, payment_status) VALUES (:reservation_id, :payment_method, :status)";
