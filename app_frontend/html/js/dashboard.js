...
function centerMapOnUser() {
    if (navigator.geolocation) {
        ...
        async function loadPlates(){
            ...
            const decoded = TokenUtils.decode(token);
            if(!TokenUtils.isValid(decoded)) return;
            const data = await fetchData(`${BASE_API_URL}/get_user_plates.php`, 'POST', { id_user: decoded.user_id });
            const platesList = document.getElementById('platesList');
            platesList.innerHTML = '<option value="default" disabled selected>Seleziona una targa</option>';
            data.data.forEach(plate => {
                const option = document.createElement('option');
                option.value = plate.targa;
                option.textContent = `${plate.targa.slice(0, 3)}***${plate.targa.slice(-3)}`;
    ...
    async function confirmReservation() {
        ...
        try {
            const response = await fetchData(`${BASE_API_URL}/add_reservation.php`, 'POST', {
                id_user: decoded.user_id,
                total_cost: total_cost,
                slot_location: slot_location,
                start_time: start_time,
